...
// 공간대여 오픈 API
function space_list_show () {
	$('#spaceListPopup').fadeIn(200);
	ajaxContentLoadSpace(1);
}

function ajaxContentLoadSpace(offset){
	if(offset == null || offset == "") offset = 1;
	...
	$.ajax({
		...
		async: false,
		cache: false,
		url: 'https://media-place.withstatic.com/HOST_LIST/host_list.json',
		dataType: 'json',
		success: function (data) {
			var dataLength = data.totalCount;
			$('#spaceListPopup ul li').remove();
			for (i = 0 ; i < dataLength ; i++) {
				var spaceData = data.hostList[i],
					html = '<li>' + spaceData + '</li>';
				$('#spaceListPopup ul').append(html);
			}
		},
		error: function (request, status, error) {
			console.log(request, status, error);
...
// 공간대여  제공업체 리스트 팝업 - close
function space_list_close(){
	$('#spaceListPopup').fadeOut(200);
	$(window).unbind("touchmove"); // 모바일 스크롤 해제
}

// 모바일 티켓 오픈 API
function ticket_list_show () {
	$('#ticketListPopup').fadeIn(200);
	ajaxContentLoadTicket(1);
}

function ajaxContentLoadTicket(offset){
	if(offset == null || offset == "") offset = 1;
	...
	$.ajax({
		...
		async: false,
		cache: false,
		url: 'https://activity-image.withstatic.com/host_list.json',
		dataType: 'json',
		success: function (data) {
			var dataLength = data.totalCount;
			$('#ticketListPopup ul li').remove();
			for (i = 0 ; i < dataLength ; i++) {
				var ticketData = data.hostList[i],
					html = '<li>' + ticketData + '</li>';
				$('#ticketListPopup ul').append(html);
			}
		},
		error: function (request, status, error) {
			console.log(request, status, error);
...
// 모바일 티켓 제공업체 리스트 팝업 - close
function ticket_list_close(){
	$('#ticketListPopup').fadeOut(200);
	$(window).unbind("touchmove"); // 모바일 스크롤 해제
}
